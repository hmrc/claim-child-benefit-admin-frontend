...
class SupplementaryDataSubmissionsControllerSpec
  ...
  "onPageLoad" - {
    ...
    "must redirect to login when the user is not authenticated" in {
      ...
      redirectLocation(result).value mustEqual s"/internal-auth-frontend/sign-in?continue_url=%2Fclaim-child-benefit-admin-frontend%2Fsupplementary-data%2Fsubmissions"
    }
